...
class SNAALoss(nn.Module):
    """Scale-Neighbor Aware Attraction Loss for tiny + dense object detection.

    Replaces the standard IoU similarity term with an attraction objective that:
    1. Preserves IoU for overall overlap constraint
    2. Uses scale-normalized center deviation for tiny box stability
    3. Penalizes predictions that drift toward neighboring same-class objects

    Args:
        kappa (float): Neighbor distance bandwidth (controls neighbor influence range).
        tau (float): Center deviation bandwidth (controls scale normalization strength).
        beta (float): Neighbor repulsion strength.
        gamma (float): IoU focusing exponent.
        alpha_max (float): Maximum scale weight to prevent extreme upweighting.
        margin (float): Repulsion margin (normalized distance threshold).
    """
    ...
    def forward(
        ...
        # 1. Find nearest same-class neighbor for each GT
        ...
        # Same-class mask: (b, max_gt, max_gt)
        same_cls = (gt_labels.unsqueeze(2) == gt_labels.unsqueeze(1)).squeeze(-1)
        eye_mask = torch.eye(max_gt, device=device).unsqueeze(0).bool()
        # Mask out self and invalid GTs (set large distance)
        dist = dist + (~same_cls | eye_mask | ~gt_valid.unsqueeze(2) | ~gt_valid.unsqueeze(1)).float() * 1e6

        # Nearest same-class neighbor index and center: (b, max_gt)
        nn_idx = dist.argmin(dim=-1)
        nn_mask = (dist.min(dim=-1).values < 1e5)  # valid nearest neighbor exists
        nn_idx_safe = nn_idx.clamp(0, max_gt - 1)
        batch_idx = torch.arange(gt_bboxes.shape[0], device=device).unsqueeze(1)
        nearest_c = gt_centers[batch_idx, nn_idx_safe]  # (b, max_gt, 2)
        ...
        # Compute GT sizes for scale normalization
        tgt_wh = (tgt_fg[..., 2:] - tgt_fg[..., :2]).clamp(min=eps)
        s = (tgt_wh.prod(-1)).clamp(min=eps).sqrt().unsqueeze(-1)  # (n_fg, 1)

        # Get nearest neighbor center for each foreground target
        tgt_gt_idx_fg = target_gt_idx[fg_mask]  # (n_fg,)
        fg_indices = torch.where(fg_mask)
        batch_fg = fg_indices[0]  # (n_fg,)
        nn_c = nearest_c[batch_fg, tgt_gt_idx_fg]  # (n_fg, 2)

        # Target center
        tgt_c = (tgt_fg[..., :2] + tgt_fg[..., 2:]) / 2  # (n_fg, 2)
        pred_c = (pred_fg[..., :2] + pred_fg[..., 2:]) / 2

        # 3. Compute SNAA components
        # IoU term (clamp for numerical stability)
        iou = bbox_iou(pred_fg, tgt_fg, xywh=False, CIoU=True).clamp(min=0, max=1)

        # Scale-normalized center deviation (clamp exponent)
        center_dist_sq = ((pred_c - tgt_c) ** 2).sum(-1, keepdim=True)
        center_dev = center_dist_sq / (2 * self.tau ** 2 * s ** 2 + eps)
        ...
        A = iou_term * torch.exp(-center_dev) * torch.exp(-self.beta * repulsion.clamp(max=20.0))
        ...
        # Weighted loss — normalize by target_scores_sum (consistent with box/cls/dfl losses)
...
class v8DetectionLoss:
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        return (
            ...
            loss,
            loss.detach(),
        )  # loss(box, cls, dfl) or loss(box, cls, dfl, snaa)
