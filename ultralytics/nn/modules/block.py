...
class DSBottleneck(nn.Module):
    """Depthwise Separable Bottleneck for lightweight feature processing.

    Uses depthwise separable convolutions (depthwise + pointwise) instead of standard
    convolutions to significantly reduce parameters and FLOPs while maintaining representational
    capacity. Suitable for selective slimming of high-level (P4/P5) branches.
    """

    def __init__(self, c1: int, c2: int, shortcut: bool = True, g: int = 1, k: tuple = (3, 3), e: float = 0.5):
        """Initialize DSBottleneck with depthwise separable convolutions."""
        super().__init__()
        c_ = int(c2 * e)
        # Depthwise separable: depthwise conv + pointwise conv
        self.cv1 = nn.Sequential(
            nn.Conv2d(c1, c1, k[0], 1, k[0] // 2, groups=c1, bias=False),
            nn.BatchNorm2d(c1),
            nn.SiLU(),
        ...
        self.cv2 = nn.Sequential(
            nn.Conv2d(c_, c_, k[1], 1, k[1] // 2, groups=c_, bias=False),
            nn.BatchNorm2d(c_),
            nn.SiLU(),
...
class C3k2Lite(C2f):
    """Lightweight C3k2 variant using depthwise separable bottlenecks.

    Drop-in replacement for C3k2 with significantly fewer parameters.
    Recommended for P4/P5 neck branches in the selective slimming strategy.
    Keep original C3k2 for P3 to preserve small object detection capacity.
    """
    ...
    def __init__(
        ...
        """Initialize C3k2Lite with depthwise separable bottlenecks."""
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(DSBottleneck(self.c, self.c, shortcut, g) for _ in range(n))


class P2Proxy(nn.Module):
    """P2 Proxy Detail Branch.

    Lightweight CSP-style branch that processes P2-level (1/4 resolution) features
    to preserve fine-grained vehicle details (edges, textures) for small object detection.
    Used as a proxy branch — not a formal detection head.
...
class ProxyFuse(nn.Module):
    """Anti-aliasing Semantic Backflow Fusion.
    ...
    Output is projected to c2 channels via 1x1 conv, matching the target detection scale.
    ...
    def __init__(self, c_p2: int, c_p3: int, c2: int | None = None, gate_ratio: int = 4):
        ...
        self.reduce = Conv(c_p3, c2, 1, 1)
        # Semantic gate: P3 global context -> channel attention (operates on full backbone channels)
        self.gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(c_p3, max(c_p3 // gate_ratio, 8), 1, bias=False),
            nn.SiLU(),
            nn.Conv2d(max(c_p3 // gate_ratio, 8), c2, 1, bias=False),
            nn.Sigmoid(),
        )

    def forward(self, x: list[torch.Tensor]) -> torch.Tensor:
        """Forward pass: fuse P2 proxy features into P3 with semantic gating.
        ...
        g = self.gate(p3)
        # Gated fusion: P3_proj + gate * P2_detail
        return p3_proj + p2_aligned * g


class NDA(nn.Module):
    """Neighbor Decoupling Adapter.

    Enhances local contrast to improve separation between adjacent vehicles.
    Uses local average pooling to compute neighborhood context, then subtracts it
    to highlight per-object local differences. Lightweight: just pooling + 1x1 conv.

    Args:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels (target detection scale channels).
        kernel_size (int): Size of the local average pooling kernel.
    """

    def __init__(self, c1: int, c2: int, kernel_size: int = 5):
        """Initialize NDA with local contrast enhancement."""
        super().__init__()
        self.pool = nn.AvgPool2d(kernel_size=kernel_size, stride=1, padding=kernel_size // 2, count_include_pad=False)
        self.cv1 = Conv(c1, c2, 1, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass: local contrast = x - avg_pool(x), then refine."""
        return self.cv1(x - self.pool(x))
