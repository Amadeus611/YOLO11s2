        ...
        for m in self.model:
            if m.f != -1:  # if not from previous layer
                if isinstance(m.f, int):
                    ...
                    # Single-element list: unwrap to avoid wrapping module input in a list
                    x = y[m.f[0]] if m.f[0] != -1 else x
                else:
                    x = [x if j == -1 else y[j] for j in m.f]  # multi-input from earlier layers
            if profile:
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in repeat_modules:
                ...
                n = 1
            if m in (C3k2, C3k2Lite):  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
